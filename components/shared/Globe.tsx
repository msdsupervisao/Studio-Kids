...
/**
 * Globo 3D rotativo — adaptado do componente Globe (originkit.dev, feito
 * para Framer, mas ja portavel: sem RenderTarget/Framer-only APIs).
 * Duas mudancas em relacao ao original:
 * - Os dados de fronteiras terrestres (GeoJSON) vinham de um repositorio
 *   pessoal no GitHub, buscado em tempo real a cada montagem — troca de
 *   dependencia externa fragil (fora do nosso controle, pode sumir/mudar
 *   a qualquer momento) por copia propria em /public/data/land-50m.json.
 * - Removido o `console.log` de debug do carregamento.
 */
...
function parseColorToRgba(input: string): Rgba {
  ...
  if (rgbaMatch) {
    const r = Math.max(0, Math.min(255, parseFloat(rgbaMatch[1] as string))) / 255;
    const g = Math.max(0, Math.min(255, parseFloat(rgbaMatch[2] as string))) / 255;
    const b = Math.max(0, Math.min(255, parseFloat(rgbaMatch[3] as string))) / 255;
  ...
  if (hex.length === 4) {
    return {
      r: parseInt((hex[0] as string) + hex[0], 16) / 255,
      g: parseInt((hex[1] as string) + hex[1], 16) / 255,
      b: parseInt((hex[2] as string) + hex[2], 16) / 255,
      a: parseInt((hex[3] as string) + hex[3], 16) / 255,
    };
  }
  if (hex.length === 3) {
    return {
...
function simplifyRing(ring: number[][], detail: number): number[][] {
  ...
  simplified.push(ring[0] as number[]);
  for (let i = stepSize; i < ring.length - 1; i += stepSize) {
    const idx = Math.min(i, ring.length - 1);
    simplified.push(ring[idx] as number[]);
  }
  const lastPoint = ring[ring.length - 1] as number[];
  const firstPoint = ring[0] as number[];
...
export function Globe({
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (showOutline && outlineColor && outlineRgba.a > 0) {
          ...
          landFeatures.features.forEach((feature: Feature) => {
            ...
            const featureType = props.featurecla || props.type || "";
            ...
            const pathString = pathGenerator(feature as GeoPermissibleObjects);
            ...
            if (geometry.type === "Polygon" && geometry.coordinates.length > 0) {
              ...
              geometry.coordinates.forEach((polygon) => {
                if (polygon.length > 0) processRing(polygon[0] as number[][]);
              });
            }
          });
        }
        ...
        if (fill === "solid") {
          ...
          if (dotCoordinates.length > 0) {
            ...
            for (let i = 0; i < dotCoordinates.length; i++) {
              const [lng, lat] = dotCoordinates[i] as [number, number];
              const pos = latLngToPosition(lat, lng);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- props primitivos ja cobrem as dependencias reais; dots/markerConfig sao objetos recriados a cada render do caller de proposito (ver uso em Globe com literal inline)
  }, [
    speed,
    smoothing,
    fill,
  ...
  if (error) {
    return <div style={containerStyle} aria-hidden />;
  }
